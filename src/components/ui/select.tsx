...
function SelectValue({ className, ...props }: SelectPrimitive.Value.Props) {
  // block + min-w-0 + truncate is load-bearing: inside the flex SelectTrigger,
  // a flex-1 child with default min-width:auto sizes to min-content (the longest
  // unbreakable token), which can exceed the container. min-w-0 lets the flex
  // track actually shrink; block enables text-overflow:ellipsis (which doesn't
  // apply to flex containers); truncate stamps the overflow contract.
  return (
    <SelectPrimitive.Value
      data-slot="select-value"
      className={cn("block flex-1 min-w-0 truncate text-left", className)}
      {...props}
    />
  )
}
